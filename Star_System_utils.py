...
class LocalAction:
    ...
    def validate(self):
        ...
        if not (is_integer(self.system_r) or is_integer(self.system_q)):
            self.status = "Invalid"
            self.status_explanation = "Failed to parse system coordinates"
            return
        elif not is_coordinate_on_map((int(self.system_r), int(self.system_q)), all_systems.shape):
            self.status = "Invalid"
            self.status_explanation = "Parsed coordinate is off the map"
            return
        elif all_systems[(int(self.system_r), int(self.system_q))] is None:
            self.status = "Invalid"
            self.status_explanation = "Parsed coordinate is of empty space on the map"
            return
        ...
        self.status = "Valid"
        self.status_explanation = "Valid for insertion in the execution queue"
        return
    ...
    def coordinates_s(self) -> str:
        return f"{int(self.system_q)}, {int(self.system_r)}"
...
def v_build_fleet_unit(investment: Dict[str, object]) -> int:
    pass


def v_disband_fleet_unit(investment: Dict[str, object]) -> int:
    pass
